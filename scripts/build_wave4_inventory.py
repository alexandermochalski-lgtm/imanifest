"""Build Entrepedia Wave 4 inventory from curated product desks scrape.

Run: python scripts/build_wave4_inventory.py
Input: data/entrepedia-wave4/scrape-desks.json
...
def main() -> None:
            ...
            rows.append(
                {
                    ...
                    "status": "queued",
                    "staging_hint": f"iMU-import\\\\wave4\\\\batch-{desk['batch']}\\\\{desk['slug']}",
                }
            )
    ...
    payload = {
        "wave": 4,
        "generated_by": "scripts/build_wave4_inventory.py",
        "note": "Investing / health-wellness / wealth gap desks. Download via download_url fields.",
    ...
    print(json.dumps({"products": len(rows), "desks": len(desks), "batches": payload["by_download_batch"], "categories": payload["by_imu_category"]}, indent=2))
