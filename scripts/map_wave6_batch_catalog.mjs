/**
 * Map Wave 6 Batch N Blob assets into overlay courses + books.
 * Usage: node scripts/map_wave6_batch_catalog.mjs 1
 */
...
const STAGING = path.join(process.env.USERPROFILE || "", "OneDrive", "Desktop", "iMU-import", "wave6", `batch-${BATCH}`);
...
const ALL_DESKS = {
  "content-engine-desk": {
    ...
    duration: "Self-paced",
    level: "Practitioner",
    flagshipPdf: /content marketing|repurpos|seo|aeo|long.form|storytell/i,
  },
  "security-defense-desk": {
    id: "c-w6-security",
    bookId: "b-w6-security",
    ...
    duration: "Self-paced",
    level: "Foundation",
    flagshipPdf: /security|phishing|protect|gdpr|digital life|cyber/i,
  },
  "reputation-trust-desk": {
    id: "c-w6-reputation",
    bookId: "b-w6-reputation",
    ...
    duration: "Self-paced",
    level: "Practitioner",
    flagshipPdf: /reputation|review|social proof|customer experience|trust|google business/i,
  },
  "audience-monetization-desk": {
    id: "c-w6-audience",
    bookId: "b-w6-audience",
    ...
    duration: "Self-paced",
    level: "Practitioner",
    flagshipPdf: /monetiz|micro audience|workshop|micro product|founder story|expertise/i,
  },
};
...
const DESKS = Object.fromEntries(
  Object.entries(ALL_DESKS).filter(([slug]) => {
    try {
      const inv = JSON.parse(readFileSync(path.join(process.cwd(), "data", "entrepedia-wave6", "inventory.json"), "utf8"));
...
function main() {
  ...
  for (const raw of report.assets) {
    ...
    const live = byUrl.get(raw.url) || {
      id: `m-w6b${BATCH}-${Math.random().toString(36).slice(2, 8)}`,
      title: raw.title,
      kind: raw.kind,
  ...
  const outDir = path.join(process.cwd(), "data", "entrepedia-wave6");
