...
import { getProfileByHandle, initialsFromName, listFollowing, type SocialProfile } from "@/lib/social";
...
export default async function FollowingPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  const people = await listFollowing(profile.userId);
  ...
  return (
    <main className="mx-auto max-w-2xl">
      ...
      <h1 className="mt-3 font-[family-name:var(--font-cormorant)] text-3xl text-white">Following</h1>
      <div className="mt-6 overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--panel)]">
        {people.length === 0 ? (
          <p className="px-5 py-10 text-center text-sm text-muted">Not following anyone yet.</p>
        ) : (
